  ...
  switch (true) {
    ...
    case hasPriorityAndStatusProperties(request.query):
      ...
      SELECT * FROM todo  WHERE status = '${status}' AND priority = '${priority}';`;
          ...
          response.send(
            data.map((eachItem) => convertTodoDBToResponseObject(eachItem))
          );
        } else {
          response.status(400);
...
app.get("/todos/:todoId/", async (request, response) => {
  ...
  response.send(convertTodoDBToResponseObject(responseResult));
...
app.get("/agenda/", async (request, response) => {
  ...
  if (isMatch(date, "yyyy-MM-dd")) {
    ...
    //console.log(responseResult);
    response.send(
      responseResult.map((eachItem) => convertTodoDBToResponseObject(eachItem))
    );
  } else {
    response.status(400);
